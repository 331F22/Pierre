...
const CurrentEntries = () => {
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////
  // READ (GET) BANNED USERS
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_HOST}/api/readban`).then((response) => {
      setEntryList(response.data)
    })
  }, [])
  ...
  // DELETE
  const deleteEntryban = (email) => { // deletes ALL such email instances in the database
    axios.delete(`${process.env.REACT_APP_HOST}/api/deleteban/${email}`).then((response) => {
      let objToDelete = getObjectByValue(email)
      const index = entryList.indexOf(objToDelete) // deletes ONE instance in the state var
      if (index > -1) {
        let entryListCopy = [...entryList] // copy
  ...
  // UPDATE (PUT)
  const updateEmailban = (email) => { // replaces ALL such email instances in the database
    axios.put(`${process.env.REACT_APP_HOST}/api/updateban`, { old: email, new: newEmail }).then((response) => {
      let objToChange = getObjectByValue(email)
      const index = entryList.indexOf(objToChange)  // deletes ONE instance in the state var
      objToChange.email_address = newEmail
      if (index > -1) {
    ...
    setNewEmailban('') // clear all update email input fields
    let updateInputs = document.getElementsByClassName('updateInputban');
    for (let i = 0; i < updateInputs.length; i++) {
      updateInputs[i].value = ''
    }
  }
      ...
      <div className='userData'>
        {entryList.map((val, k) => {
          return (<div key={k}>
            ...
            <div className="editControls editGui">
              <button className='delete' onClick={() => {
                deleteEntryban(val.email_address)
              }}>delete</button>

              <button className='update' onClick={() => {
                if (newEmail.length > 0) {
                  updateEmailban(val.email_address);
                }
              }}>update</button>
